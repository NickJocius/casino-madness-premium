...
function makeProfile(overrides: Partial<{ bank: number; deletedAt: Date | null; userId: string }> = {}) {
  return {
    id: 'profile_1',
    userId: 'user_1',
    city: null,
...
describe('startGame — validation', () => {
  it.each([0, -100, 100_001, 1.5])('rejects invalid bet %p with VALIDATION_ERROR', async (badBet) => {
...
describe('playerAction — validation', () => {
  ...
  it.each([0, null, undefined, {}])('rejects non-enum input %p with VALIDATION_ERROR', async (bad) => {
    mockedAuth.api.getSession.mockResolvedValue(makeAuthSession());
